...
export default function FinalCtaSection({ hospitalName }) {
  ...
  const crossRotate = useTransform(scrollYProgress, [0, 1], [0, 90]);
  const crossY = useTransform(scrollYProgress, [0, 1], [-20, 20]);

  return (
    <section ref={sectionRef} id="contact" className="relative pt-2 sm:pt-4 pb-20 lg:pt-6 lg:pb-28 overflow-hidden bg-white">
      ...
      {/* Floating 3D Geometric Medical Cross SVG with Scroll Rotational Parallax */}
      <motion.div
        style={{ rotate: crossRotate, y: crossY }}
        className="pointer-events-none absolute -right-12 top-1/4 w-72 h-72 opacity-15"
      >
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {/* Quick Contact & Emergency Strip with Stagger */}
        <motion.div
          initial={{ opacity: 0, y: 25 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: 0.2, duration: 0.6 }}
          className="p-6 rounded-3xl glass-panel border border-white max-w-3xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-6 text-left"
        >
          <div>
